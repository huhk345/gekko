...
const marketData = require('./bitbank-markets.json');
...
const Trader = function(config) {
    ...
    const ApiConfig = {
        ...
        timeout: 180000, // optional, default 3000
    };
    this.bitbank = new Bitbank.PublicApi(ApiConfig)
...
Trader.prototype.getTrades = function(since, callback, descending) {
        ...
        _.each(
            ...
            function(trade) {
                parsedTrades.push({
                    ...
                    date: moment(trade.executed_at).unix(),
    ...
    let timer = setTimeout(() => {
        ...
        timoutFlag = true
        this.retry(this.getTrades, args, new Error())
    }, 240000)
    this.bitbank.getTransactions(reqData).then(res => {
        if (timoutFlag) {
            return
        }
        console.log('success!')
        ...
        console.log('erroe')
        clearTimeout(timer)
        this.retry(this.getTrades, args, err);
    })
};
